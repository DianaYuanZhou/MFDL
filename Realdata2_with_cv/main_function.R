...
main.wide <- function(index, sim.index = c('APOE', 'ACE_QC4'), covariates = NULL, ninput = 2, K_fold = 3, activation = tanh, activation2 = tanh, method = "Continue", name.folder = F){
  ...
  #   select(sample_index, Age_bl) %>% unique()
  # 
  ...
  for (k in 1:length(lambda1.wide)){
    ...
    for (i in 1:K_fold){
      ...
      G.train.wide <- list(G1.train, G2.train)
  ...
  G.wide <- list(G1, G2)
  G.test.wide <- list(G1.test, G2.test)
