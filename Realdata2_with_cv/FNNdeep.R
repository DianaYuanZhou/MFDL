FNNdeep0 <- function(D0, Y, Bases, G.valid = NULL, Y.valid = NULL, lr = 0.1, 
  ...
  if ((valid.goal == 0) && (!is.null(Y.valid))) {
    k <- 0
    Y.valid.hat <- pred2(parameters,Y.valid, G.valid, Bases, activation, activation2, nhidden = nhidden)
    ...
    result.best <- list(parameters = parameters, j = 0,
                        train.error = mse(Y[,1], Y.train.hat))
  } else {
    valid.error.best <- 0
    valid.error <- NULL
    k <- NULL
...
FNNdeep <- function(D0, Y, Bases, G.valid = NULL, Y.valid = NULL, lr = 0.3, 
    ...
    for (i in 1:length(lambda1)) {
      ...
      if (!is.null(Y.valid)){
        Y.valid.hat <- pred2(result$parameters, Y.valid, G.valid,
                             Bases, activation, activation2, nhidden = nhidden)
        valid.error[i] <- mse(Y.valid[,1], Y.valid.hat)
      }else{
        Y.train.hat <- pred2(result$parameters, Y, D0,
                             Bases, activation, activation2, nhidden = nhidden)
        valid.error[i] <- mse(Y[,1], Y.train.hat)       
      }
