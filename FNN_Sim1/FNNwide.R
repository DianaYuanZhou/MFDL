FNNwide0 <- function(D0, Y.wide, Bases.pre, Bases.wide, G.valid = NULL, Y.valid = NULL, lr.wide, 
  ...
  for (j in 1:epoch) {
    ...
    grads.pre = BP.pre(Y.wide, parameters.pre, cache.pre, D1.input, B1 = B11.pre, Bases0.pre, parameters, cache, Bases0,
                              activation, activation.prime, activation2, activation2.prime, 
                              lambda1 = 0, lambda2 = 0, 
    ...
    if (j %% verbose == 0) {
      ...
      if (!is.null(Y.valid)) {
        C3 <-pred.wide(parameters, G.valid, Bases0 = Bases0, Bases0.pre = Bases.pre, activation, activation2, nhidden.wide,
...
FNN.wide <- function(D0, Y.wide, Bases.pre, Bases.wide, G.valid = NULL, Y.valid = NULL, lr.wide, 
  ...
  for (i in 1:length(lambda1)) {
    ...
    Y.valid.hat <- pred.wide(result$parameters, G.valid,
                             Bases0, Bases0.pre,activation, activation2, nhidden.wide,
