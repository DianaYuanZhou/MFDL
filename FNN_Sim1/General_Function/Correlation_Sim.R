...
# ------------------ flm -------------------------
Corr.flm <- function(index, lambda1, G1.train, G2.train, Y.train, G1.test, G2.test, Y.test, pos1, pos2, 
                     ratio = 0.1, ratio2 = 0.1, valid = T){
  set.seed(index)
...
## FNN wide
Corr.FNNwide <- function(parameters, G, Y, index, Bases.wide, activation, activation2, nhidden,
  ...
  Y.hat <- pred.wide(parameters, G, Bases0 = Bases0, activation, activation2, nhidden = nhidden,
