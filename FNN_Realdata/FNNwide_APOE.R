...
learning_rate <- list("1hl" = 5,
                      "2hl" = c(5,5),
                      "3hl" = c(5, 5, 5))

main.wide <- function(index, lr.nn = 5, ns.nn = 250, lr.fnn = learning_rate, ns.fnn = c(250,250,250), nhidden= 3,
                      epoch = c(3e4, 3e4), lambda1 = 0, lambda2 = 0, noise = 0.1,name.folder = F, activation = tanh,  
                      lr.pre = c(1, 1), lr.wide = c(1, 1), ns.wide = 50, lambda1.wide = c(0.1,0.3,1,3,10,30), ninput = 2, nhidden.pre = 1, nhidden.wide = 1){

  set.seed(index)
  ## ---------- Real Data ------------------
  list2env(readRDS(file = 'C:/Users/Yuan/Dropbox/ADNI_Data/Data_UCSF/APOE.rds'), envir = environment())
  ...
  bb0 <- create.bspline.basis(norder = 5, nbasis = ceiling(length(pos)*0.4))
  ...
  Bases.nn <- list(B0 = B0.nn, B1 = B1.nn, B2 = B2.nn)
   ...
   ## ----------nn----------
    ...
    # NN <- c(Corr.fnn(session.nn$parameters, G, Y, error.index,
    #                        Bases.nn, activation), session.nn$j)
    # 
    # 
  ...
  # for(i in 1:nhidden){
  # 
  # B1.fn <- diag(nrow = ns.fnn[i], ncol = ns.fnn[i])
