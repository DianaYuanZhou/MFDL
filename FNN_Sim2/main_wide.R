...
main.wide <- function(index, sim.index = 3, activation = tanh, method = "Continue", name.folder = F){
  ...
  setting <- list('input' = setting.input[[sim.index]][[2]],
  ...
  # Get phenotype data
  list2env(Function.Output(setting, index, G1, G2, pos1, pos2, ratio1 = 0.2, ratio2 = 0.2), envir = environment())
  Y <- Data.Output.wide(f1, f2, loc, index, inter = setting$interaction, noise = noise)
  ...
  ## -----------FNN deep ------------
      ...
      session.fn2 <- FNNdeep(G.subtrain, Y.subtrain, Bases.fn2, G.valid, Y.valid, lr = lr.fnn[[i]],
                        epoch = epoch[1], lambda1 = lambda1, activation = activation, method = method,
                        penmat = penmat/penmat[1, 1], nhidden = ns.fnn[[i]], ADADELTA = ADADELTA)
      FN2 <- c(Error.fnn2(session.fn2$parameters, cbind(G1,G2), Y, error.index,
                          Bases.fn2, activation, nhidden = ns.fnn[[i]]), session.fn2$j)
      endtime <- Sys.time()
      ...
      FN2 <- c(Corr.fnn2(session.fn2$parameters, cbind(G1,G2), Y, error.index,
                         Bases.fn2, activation, nhidden = ns.fnn[[i]]))
  ...
  session.fnnwide <- FNN.wide(D0 = G.subtrain.wide, Y.wide = Y.subtrain, Bases.pre = Bases.pre, Bases.wide = Bases.wide, G.valid = G.valid.wide, Y.valid = Y.valid, lr.wide = lr.wide,
                            epoch = epoch[2], lambda1 = lambda1.wide, activation = activation, method = method,
                            penmat = penmat/penmat[1, 1],
                            lr.pre = lr.pre, ninput = ninput, nhidden.wide = nhidden.wide, nhidden.pre = nhidden.pre, ADADELTA = ADADELTA)
  G.wide <- list(G1, G2)
