FNNwide0 <- function(D0, Y.wide, Bases.pre, Bases.wide, G.valid = NULL, Y.valid = NULL, lr.wide, 
                  ...
                  valid.goal = 0, parameters = NULL,parameters.pre = NULL, verbose = 5, p = 100,
  ...
  cache.pre <- FP.pre(D1.input, parameters.pre, B1 = B11.pre, activation, ninput = ninput, nhidden.pre)
  ...
  cache <- FP.wide(H1 = H3[[1]], parameters, B1.wide, B11.wide, activation, nhidden.wide)


  #-------BP and training part----------------- 
  if ((valid.goal == 0) && (!is.null(Y.valid))) {
    k <- 0
    Y.valid.hat <- pred.wide(parameters, D0 = G.valid, Bases0, Bases0.pre, activation, nhidden.wide,
    ...
    result.best <- list(parameters = parameters, parameters.pre = parameters.pre, j = 0, 
                        train.error = mse(Y.wide, cache$C3 %*% t(B2)))
  } else {
    valid.error <- NULL
    k <- NULL
  }
  ...
  for (j in 1:epoch) {
    ...
    #parameters <- update.wide(parameters, grads, lr.wide = lr.wide, nhidden = nhidden)
    grads.pre = BP.pre(Y.wide, parameters.pre, cache.pre, D1.input, Bases0.pre, parameters, cache,
                              activation, activation.prime, 
                              lambda1 = 0, lambda2 = 0, 
    ...
    #parameters.pre<- update.pre(parameters.pre, grads.pre, lr.pre = lr.pre, ninput, nhidden.pre = nhidden.pre)
		cache.pre <- FP.pre(D1.input, parameters.pre, B1=B11.pre, activation, ninput, nhidden.pre = nhidden.pre)
		
		H3[[1]] <-cache.pre$H2.pre[[1]][[nhidden.pre]]
		if (ninput >=2){
		for (i in 2:ninput){
	      H3[[1]]<-cbind(H3[[1]], cache.pre$H2.pre[[i]][[nhidden.pre]])
		}
		}
    ...
    if (j %% verbose == 0) {
      ...
      if (!is.null(Y.valid)) {
        C3 <-pred.wide(parameters, G.valid, Bases0 = Bases0, Bases0.pre = Bases.pre, activation, nhidden.wide,
...
FNN.wide <- function(D0, Y.wide, Bases.pre, Bases.wide, G.valid = NULL, Y.valid = NULL, lr.wide, 
                epoch = 3e3, lambda1 = 0, lambda2 = 0, activation = sigmoid, method = method,
                valid.goal = 0, parameters = NULL,parameters.pre = NULL,verbose = 10, p = 100,
  ...
  for (i in 1:length(lambda1)) {
    ...
    Y.valid.hat <- pred.wide(result$parameters, G.valid,
                             Bases0, Bases0.pre,activation, nhidden.wide,
                             result$parameters.pre,  ninput, nhidden.pre) %*% t(Bases0$B2)
