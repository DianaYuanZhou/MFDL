forward.propagation2 <- function(D1, parameters, B1, activation, nhidden){
  N <- nrow(D1)
  ...
  if (n >= 2){
	 ...
	 #D[[i]] <- activation(H[[i-1]]) %*% B1 / nrow(B1) * ncol(B1)
	   D[[i]] <- relu(H[[i-1]]) %*% B1[[i-1]] / nrow(B1[[i-1]]) * ncol(B1[[i-1]])
     C[[i]] <- D[[i]] %*% W[[i]] + matrix(rep(b[[i]], N), nrow = N, byrow = T)
     H[[i]] <- C[[i]] %*% t(B1[[i]])
     }
  }
  
  D3 <- activation(H[[n]]) %*% B1[[n]] / nrow(B1[[n]]) * ncol(B1[[n]])
