...
Error.flm <- function(index, lambda1.flm, K_fold, G.train, Y.train, G.test, Y.test,
                      Bases, valid = T, ratio){
  allindex <- sample(nrow(G.train))
  folds <- cut(allindex,breaks=K_fold,labels=FALSE)
  
  test.mse <- seq(length(lambda1.flm))
  for(i in lambda1.flm){
    test.mse.cv <- seq(K_fold)
...
Error.nn <- function(parameters, G.train, Y.train, G.test, Y.test, Bases, activation){
  Y.hat.train <- pred(parameters, G.train, Bases, activation) %*% t(Bases$B2)
  error.train <- mse(Y.train[,1], Y.hat.train)
  Y.hat.test <- pred(parameters, G.test, Bases, activation) %*% t(Bases$B2)
  error.test <- mse(Y.test[,1], Y.hat.test)  
  error.nn <- c(error.train, error.test)
  return(error.nn)
}
...
Error.fnn2 <- function(parameters, G.train, Y.train,G.test, Y.test, Bases, activation, activation2, nhidden){
  #Y.hat <- pred2(parameters, G, Bases, activation, activation2, nhidden =nhidden) %*% t(Bases$B2)
  ...
  error.test <- mse(Y.test[,1], Y.hat.test)
  error.fnn2 <- c(error.train, error.test)
  return(error.fnn2)
}
...
Error.FNNwide <- function(parameters, G.wide.train, Y.train, G.wide.test, Y.test, Bases.wide, activation, activation2, nhidden,
                          parameters.pre, Bases.pre,ninput, nhidden.pre){
  ...
  Y.hat.test <- pred.wide(parameters, Y.test, G.wide.test, Bases.wide, Bases.pre, activation, activation2, nhidden = nhidden,
                           parameters.pre = parameters.pre,  ninput = ninput, nhidden.pre = nhidden.pre)
  error.test <- mse(Y.test[,1], Y.hat.test)
  error.FNNwide <- c(error.train, error.test)
  return(error.FNNwide)
}
